...
**免责声明：** 本软件仅提供用于自定义内容输出的技术工具。请勿利用该软件传播有害或违法内容；本软件作者对其他人制作的内容和定义文件（包括对本作者创作内容的修改）不承担任何责任。
...
{substrules}
    ...
    [/filter_cmds]
        ...
        [subst_regex] {{prefix_group}}warning: (?P<esc>({{ESC}}.*?m)*)incompatible pointer types assigning to '(?P<name1>.+)' from '(?P<name2>.+)'
            ...
            default: \g<prefix>提示: \g<esc>'\g<name1>'从不兼容的指针类型赋值为'\g<name2>',两者怎么都……都说不过去！^^;
        [/subst_regex]
...
请查看仓库中`frontend-demo`中的文件，其中包含一个样例定义文件和用于展示`frontend`模块的样例应用程序。

更多信息请见[项目文档](#文档)中的**应用程序和字符串定义API**章节。
...
> [!IMPORTANT]
> 官方发行版文件仅通过[仓库发行版页面](../../releases)提供。本作者不管理其他渠道分发的软件包（如PyPI/pip、Conda、Homebrew、AUR等）并对其内容不承担任何责任！
...
```sh
# 如果之前执行过makepkg，请删除之前生成的临时文件夹，否则构建时可能会出现问题
