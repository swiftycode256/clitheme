...
class TestErrors(unittest.TestCase):
    ...
    def test1_errors(self):
        ...
        expected_msgs['en_US']= [
            ...
            f"Error: Line {next(4)}>{c+2}: Bad substitute pattern (missing < at position 2)",
            f"Error: Line {next(0)}>{c+3}: Bad substitute pattern (invalid group reference 2 at position 3)",
            f"Error: Line {next(12)}>{c+1}[default]: Cannot create subsection \"this\" because an entry with the same name already exists",
        ...
        expected_msgs['zh_CN']=[
            ...
            f"错误：第{next(4)}>{c+2}行：无效的替换正则表达式（missing < at position 2）",
            f"错误：第{next(0)}>{c+3}行：无效的替换正则表达式（invalid group reference 2 at position 3）",
            f"错误：第{next(12)}>{c+1}[default]行：无法创建子路径\"this\"，因为拥有相同名称的定义已存在",
